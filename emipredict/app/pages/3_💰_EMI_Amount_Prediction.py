"""
EMI Amount Prediction Page - EMI-Predict AI

Predict the maximum monthly EMI amount an applicant can afford.
...
# Page configuration
st.set_page_config(
    page_title="EMI Amount - EMI-Predict AI",
    page_icon="💰",
    layout="wide"
)
...
def load_regression_model():
    """Load the trained regression model with comprehensive error handling."""
    import joblib
    from emipredict.config.settings import Config
    
    try:
        log_debug("Loading regression model...")
        ...
        if not model_path.exists():
            # Check if any regression models exist
            models_dir = Config.MODELS_DIR
            available_models = list(models_dir.glob("*_regression*.pkl"))
            
            if available_models:
                # Use the first available regression model
                ...
                log_debug(f"❌ No regression models found in {models_dir}")
...
def create_input_form():
    ...
    with st.form("emi_amount_form"):
        ...
        # Submit button
        submitted = st.form_submit_button("💰 Predict EMI Amount", use_container_width=True)
...
def make_prediction(model, processed_df):
    ...
    try:
        ...
        # Make prediction
        start_time = time.time()
        predicted_emi = model.predict(processed_df)
        elapsed = time.time() - start_time
        
        log_debug(f"Prediction completed in {elapsed:.3f}s")
        log_debug(f"Prediction type: {type(predicted_emi)}, dtype: {predicted_emi.dtype if hasattr(predicted_emi, 'dtype') else 'N/A'}")
        log_debug(f"Prediction value: {predicted_emi}")
        
        # Ensure prediction is positive
        predicted_emi = np.maximum(predicted_emi, 0)
        log_debug(f"After clipping (non-negative): {predicted_emi}")
        
        return predicted_emi
...
def display_prediction_result(predicted_emi, input_df):
    """Display prediction result with comprehensive error handling."""
    try:
        log_debug("Displaying prediction result...")
        
        st.markdown("---")
        st.subheader("💰 Predicted Maximum Monthly EMI")
        
        # Main prediction - convert to native Python float
        predicted_emi_value = float(predicted_emi[0])
        log_debug(f"Predicted EMI value: {predicted_emi_value:.2f} (type: {type(predicted_emi_value)})")
        
        st.success(f"## ₹{predicted_emi_value:,.0f}")
        st.caption("This is the maximum monthly EMI you can afford based on your financial profile.")
        ...
        # Convert all values to native Python types for Streamlit
        ...
        # EMI to salary ratio
        emi_to_salary = (predicted_emi_value / monthly_salary * 100) if monthly_salary > 0 else 0
        ...
        with col4:
            st.metric("EMI/Salary Ratio", f"{emi_to_salary:.1f}%")
        
        # Loan affordability analysis
        st.markdown("---")
        st.subheader("💡 Loan Affordability Analysis")
        
        # Get requested loan details
        requested_amount = float(input_df.iloc[0].get('requested_amount', 0))
        requested_tenure = int(input_df.iloc[0].get('requested_tenure', 60))
        
        # Calculate EMI with typical interest rate (assuming 10% per annum)
        interest_rate = 0.10 / 12  # Monthly rate
        n = requested_tenure
        if n > 0 and requested_amount > 0:
            emi_with_interest = (requested_amount * interest_rate * (1 + interest_rate)**n) / ((1 + interest_rate)**n - 1)
        else:
            emi_with_interest = 0
        
        log_debug(f"Requested amount: {requested_amount}, Tenure: {requested_tenure}, Calculated EMI: {emi_with_interest:.2f}")
        ...
        with col1:
            ...
            **Requested Loan Details:**
            ...
            - Est. EMI (@ 10% p.a.): ₹{emi_with_interest:,.0f}
            """)
        
        with col2:
            if emi_with_interest <= predicted_emi_value:
            ...
            **Affordability Assessment:**
            - Max Affordable EMI: ₹{predicted_emi_value:,.0f}
            - Required EMI: ₹{emi_with_interest:,.0f}
        ...
        if emi_with_interest <= predicted_emi_value:
            st.success(f"""
            **Great News! The requested loan is within your capacity.**
            
            **Tips for successful loan management:**
            - Keep your EMI-to-income ratio below 40%
            - Maintain a strong credit score (above 750)
            - Build an emergency fund of 6 months' expenses
            - Consider shorter tenure to save on interest
            - Make prepayments when possible
            """)
        else:
            shortage = emi_with_interest - predicted_emi_value
            adjusted_amount = (predicted_emi_value / emi_with_interest) * requested_amount if emi_with_interest > 0 else 0
            
            st.warning(f"""
            **The requested loan exceeds your comfortable EMI capacity.**
            
            **Suggested adjustments:**
            - Reduce loan amount to approximately ₹{adjusted_amount:,.0f}
            - Increase loan tenure (current: {requested_tenure} months)
            - Increase your monthly income
            - Reduce existing obligations
            - Pay down current EMIs before taking new loan
            
            **Current shortfall:** ₹{shortage:,.0f} per month
            """)
        
        # Loan amount recommendations
        st.markdown("---")
        st.subheader("💼 Loan Amount Recommendations")
        
        # Calculate affordable loan amounts for different tenures
        ...
        for tenure in tenures:
            ...
            if tenure > 0:
                ...
                affordable_principal = (predicted_emi_value * ((1 + r)**tenure - 1)) / (r * (1 + r)**tenure)
                affordable_loans.append({
                    'Tenure (months)': tenure,
                    'Tenure (years)': f"{tenure/12:.1f}",
                    'Affordable Loan Amount': f"₹{affordable_principal:,.0f}"
                })
        
        st.table(pd.DataFrame(affordable_loans))
        st.caption("*Calculations based on 10% annual interest rate")
...
def main():
    """Main function for EMI Amount Prediction page with bulletproof error handling."""
    
    st.title("💰 EMI Amount Prediction")
    st.markdown("Predict the maximum monthly EMI amount you can comfortably afford.")
    st.markdown("---")
    ...
    # STEP 2: Load model
    log_debug("=== STEP 2: Loading regression model ===")
    try:
        model, metadata, model_error = load_regression_model()
    ...
    if input_df is not None:
        ...
        try:
            ...
            with st.spinner("🔄 Step 3/3: Making prediction..."):
                ...
                try:
                    start_time = time.time()
                    predicted_emi = make_prediction(model, processed_df)
                    elapsed = time.time() - start_time
            ...
            try:
                display_prediction_result(predicted_emi, input_df)
